...
from .features import build,FEATURES,factor_summary,FACTOR_METADATA
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=['http://localhost:5173', 'http://127.0.0.1:5173', 'https://tciratepulse.vercel.app'],
    allow_methods=['*'],
    allow_headers=['*'],
)
...
def startup():
 ...
 init_db()
 if not MODEL_PATH.exists() or not META_PATH.exists() or json.loads(META_PATH.read_text()).get('feature_count') != len(FEATURES): train()
...
def quote(req:QuoteRequest):
 ...
 labels={'distance_km':'Route distance','weight_kg':'Shipment weight','sla_factor':'SLA','toll_inr':'Tolls (estimated)','seasonal_factor':'Seasonality','festival_pressure':'Festival pressure','fuel_index':'Fuel benchmark','demand_index':'Demand proxy','weather_risk':'Weather risk','calamity_risk':'Calamity risk','utilization':'Capacity utilization','product_risk':'Product risk','security_risk':'Security risk','insurance_factor':'Insurance factor','transit_hours':'Transit time'}
 ...
 out={'tariff_inr':round(p/10)*10,'low_inr':low,'high_inr':high,'confidence':'Medium' if uncertainty>.12 else 'High','distance_km':r['distance_km'],'travel_hours':r['travel_hours'],'route_source':r['source'],'weather':w,'drivers':drivers,'alerts':alerts(req,f,w),'factors':f,'factor_summary':factor_summary(f,r,w),'trend':trend,'trend_label':'Indicative trend only — not a horizon-specific forecast.','model_metrics':metrics,'model_version':metrics.get('model_version','legacy')};save_quote(req.model_dump(),out);return out
